<div class="container">
	...
	<table class="table table-condensed table-hover" id="tbl-mdr">
		<thead>
            <tr>
                <th>MDR No.</th>
                <th>Deliver To</th>
...
<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="" id="modal-mdr">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
    	<div class="modal-header">
    		<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    		<h4 class="modal-title">MDR NO <span id="header-number"></span></h4>
    	</div>
    	<div class="modal-body">
    		<?php echo form_open(null, ['id' => 'frm-modal']) ?>
    			<div class="row">
    				<div class="col-md-6">
    			...
    			<div class="form-group">
    				...
    				<input type="text" class="form-control" name="remarks" id="remarks" placeholder="Optional Remarks">
			...
			<button type="button" class="btn btn-info btn-sm" id="btn-add-item"><span class="glyphicon glyphicon-plus"></span> ADD ITEM</button>
			<button class="btn btn-success btn-sm" id="btn-dr-update"><span class="glyphicon glyphicon-floppy-disk"></span> Update</button>
			<!-- <button class="btn btn-danger btn-sm" id="btn-dr-remove"><span class="glyphicon glyphicon-trash"></span> Remove</button> -->
			<button class="btn btn-primary btn-sm" id="mdl-btn-print"><span class="glyphicon glyphicon-print"></span> Preview / Print</button>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#btn-add-item').click(function(e) {
			e.preventDefault();
			$('#modal-item .modal-title').html('ADD ITEM');
			$('#btn-save-item').show();
		...
		$('#tbl-mdr').DataTable({
			...
			serverSide: true,
			ajax: {
				url: '<?php echo site_url('main/get_all_dr') ?>',
				type: 'POST'
			},
			columns: [
				{data: 'id'},
		...
		$('#btn-dr-update').click(function(e) {
			...
			.then(function(data) {
				if (data.error != null) {
					...
					$('#frm-modal span').html('').parent('div').removeClass('has-error').addClass('has-success');
				} else {
		...
		$('#tbl-mdr').on('click', 'tbody tr', function(e) {
			e.preventDefault();
			$('#tbl-mdr tbody tr.success').removeClass('success');
			$(this).addClass('success');
			var val = $(this).find('td').html();
			$('#modal-mdr .modal-header span#header-number').text(val);
			$('#modal-mdr #number').val(val);
			$.ajax({
				url: '<?php echo site_url('main/get_dr_by_number') ?>',
				method: 'POST',
				data: {number: val},
				dataType: 'json'
			})
		...
		// $('#modal-mdr').on('hide.bs.modal', function(e) {
		// 	$('#tbl-mdr tbody tr.success').removeClass('success');
		// });
		$('#mdl-btn-print').click(function() {
			var url = '<?php echo site_url('main/mdr/') ?>'+$('input#number').val();

			window.open(url, '_blank');
		});

		$('#modal-mdr').on('hidden.bs.modal', function(e) {
			$(this).find('input').val('');
			$(this).find('span').html('').parent('div').removeClass('has-success').removeClass('has-error');
		});
		...
		$('#tbl-items tbody').on('click', '#btn-item-edit', function(e) {
			e.preventDefault();
			var edit = $(this);
			$('#modal-item .modal-title').html('UPDATE ITEM');
			$('#btn-save-item').hide();
			$('#btn-update-item').show();
			var id = edit.closest('tr').find('input#item-no').val();

			$.post('<?php echo site_url('main/get_item') ?>', {item: id}, null, 'json')
			.then(function(data) {
		...
		$('#btn-update-item').click(function(e) {
			...
			$.post('<?php echo site_url('main/update_item') ?>', {number: number, item: item, qty: qty, unit: unit, description: description}, null, 'json')
			.then(function(data) {
				if (data.error != null) {
					$.each(data.error, function(index, elem) {
						if (elem != '') {
		...
		$('#btn-dr-remove').click(function(e) {
			...
			if (conf) {
				...
				.then(function(data) {
					...
					$('.modal').modal('hide');
					$('#tbl-mdr').DataTable().ajax.reload(null, false);
				})
				.fail(function() {
					toastr.error('Cannot process request...', 'Error');
				});
